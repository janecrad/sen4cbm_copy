...
kywd = keyword_search(os.path.join(os.getcwd(), 'config/config_file.txt'), ['PG_config'])
db_conf_file = kywd['PG_config']
...
def get_value(dict_keys, var_name=None, db='main'):
...
def tables(db='main', matching_text=None, all_records=False, print_list=False):
    ...
    try:
        ...
        if matching_text is not None:
            value = config.autoselect(matching_text, list_, all_records, help_text=False)
            if all_records:
                list_ = value
            else:
...
def table_data(table, where=None, db='main', limit=1000):

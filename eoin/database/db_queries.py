...
def getParcelById(dataset, pid, ptype='', withGeometry=False,
                  wgs84=False, schema='public', geom_column='wkb_geometry'):
    #Modify to allow multiple parcels as list
    if isinstance(pid, list):
        if len(pid) == 1:
            oper = '='
            if any(p.isnumeric() for p in pid) is True:
                pid = str(pid[0])
            else:
                pid = "'" + str(pid[0]) + "'"
        else:
        ...
        print('pid should be a string or a list of strings')
        return 'Error'
    conn = db.conn(dataset['db'])
    cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    data = []
    ...
    try:
        logging.debug("start queries")
        getTableSrid = f"SELECT Find_SRID('{schema}', '{parcels_table}{ptype}','{geom_column}');"
        logging.debug(getTableSrid)
        cur.execute(getTableSrid)
        srid = cur.fetchone()[0]
        logging.debug(srid)
        ...
        if withGeometry:
            if wgs84:
                geometrySql = f", st_asgeojson(st_transform({geom_column}, 4326)) as geom"
            else:
                geometrySql = f", st_asgeojson({geom_column}) as geom"
        else:
            geometrySql = ""

        getTableDataSql = f"SELECT {parcel_id}::text as pid, {cropname} as cropname, {cropcode}::text as cropcode," \
                          f"st_srid({geom_column}) as srid{geometrySql}," \
                          f"st_area({geom_column}) as area," \
                          f"st_X(st_transform(st_centroid({geom_column}), 4326)) as clon," \
                          f"st_Y(st_transform(st_centroid({geom_column}), 4326)) as clat" \
                          f" FROM {parcels_table}{ptype}" \
                          f" WHERE {parcel_id} {oper} {pid};"
...
def getLogActivities(pid, dataset):    
    conn = db.conn(dataset['db'])
    cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    
    logs_table = dataset['tables']['logs']
    
    extractSql = f"""SELECT * FROM {logs_table} WHERE new_id = {pid}"""
    
    dfActivities = psql.read_sql(extractSql, conn)
    
    # Check for an empty dataframe
    if dfActivities.empty:
        print(f"Query returned empty transect log result for parcel {pid}")
    
    conn.close()
    
    return(dfActivities.to_dict('dict'))
